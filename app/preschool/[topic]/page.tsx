...
import { preschoolTopics, getPreschoolTopic } from "@/lib/preschool-data";
...
export function generateMetadata({ params }: Props): Metadata {
  ...
  return {
    ...
    alternates: { canonical: `${BASE_URL}/preschool/${topic.slug}` },
  };
}
...
export default function PreschoolTopicPage({ params }: Props) {
  ...
  const breadcrumbJsonLd = buildBreadcrumbJsonLd([
    ...
    { name: topic.title, url: `${BASE_URL}/preschool/${topic.slug}` },
  ]);

  const learningResourceJsonLd = buildLearningResourceJsonLd({
    title: topic.title,
    description: topic.description,
    url: `${BASE_URL}/preschool/${topic.slug}`,
    skills: [topic.title],
    ageLevelLabel: "Preschool",
  });
    ...
    <main id="main-content" className="mx-auto max-w-3xl px-6 py-12">
      <nav aria-label="Breadcrumb" className="text-sm text-chalkboard/60">
        <ol className="flex gap-2">
          ...
          <li><Link href="/preschool">Preschool</Link> /</li>
      ...
      <p className="mt-6">
        <Link href="/preschool" className="font-bold underline">
          Back to the Preschool Learning Hub
        </Link>
      </p>
